...
def split_syn_data_record_real_aug(data_seed=0, syn=True, aug_val=False):
    if syn:
        ...
        if aug_val:
            '''
            根据seed=0对real data 分割, 仅对real val set进行扩增,（不在这部分代码里）
            但是之前的顺序不能复现了，所以尽量确保不变动real val aug 这个set，
            此处仅为了记录，不对真实的数据集进行分割
            '''
    ...
    try:
        if aug_val:
            ...
            all_f = open(os.path.join(args.syn_main_dir, "all.txt"), "w")
            train_f = open(os.path.join(args.syn_main_dir, f"train_seed{data_seed}.txt"), "w")
            eval_f = open(os.path.join(args.syn_main_dir, f"val_seed{data_seed}.txt"), "w")
            train_f.write("\n".join(train_file_names))
            eval_f.write("\n".join(val_file_names))
...
def get_arg(cmt='syn_wdt_rnd_sky_rnd_solar_rnd_cam_p3_shdw_step40', syn=True, workbase_data_dir='./real_syn_wdt_vockit'):
    ...
    # tag: yang adds
    parser.add_argument("--syn_data_dir", type=str,default=f'{BASE_DIR}/data/synthetic_data_wdt', help="base path of real  data wdt")
    parser.add_argument("--syn_main_dir", type=str, default='{}/{}/ImageSets/Main',
                        help="\{syn_data_dir\}/\{cmt\}/ImageSets/Main")
    ...
    if syn:
        ...
        args.syn_main_dir = args.syn_main_dir.format(args.syn_data_dir, cmt)
    else:
...
if __name__ == '__main__':
    ...
    split_syn_data_record_real_aug(data_seed, syn, aug_val)
